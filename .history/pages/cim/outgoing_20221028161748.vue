...
export default {
  ...
  methods: {
    ...
    async test() {
      ...
      // let filtedBreakerSvg = [];
      // for (const key of svgData) {
      //   filtedBreakerSvg.push(Object.values(key[0].from.$));
      // }
      // console.log(filtedBreakerSvg);
    ...
    async test2() {
      ...
      class 路径类 {
        ...
        static 获取路径(structureData) {
          console.log(structureData);
          return new 路径类(structureData ? structureData : []);
        }
      ...
      // console.log(structureData);
      for (const structure of lineSvgList) {
        let filterData = (await 路径类.获取路径Base(structure).获取下级设备({ id: 'PD_31100000_64019' }));
        console.log(filterData);
      }
    }
  }
