    ...
    async getFirstBreaker() {
      ...
      class 路径类 {
        ...
        获取下级设备({ id, array = [], deep = 1 }) {
          ...
          const links = this.structure.result.filter(x => new Base(x.from).id === id);
      ...
      // 根据母线的svg 查出母线结构那到母线的第一个开关 将其funcType设置为outlet
      // 查出所有的母线的svg
      ...
      const pathData = new 路径类(busData);
      console.log(pathData);
      const nextDevelop = pathData.获取上级路径('PD_31100000_64019');
      console.log(nextDevelop);
    }
  }
};
</script>
