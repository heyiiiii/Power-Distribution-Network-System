...
export default {
  ...
  methods: {
    ...
    async setOutLet() {
      // 查询所有线路
      const data = await this.$axios.$post('/models/allLine/list');
      console.log('1----查到所有线路\n');
    ...
    async getFirstBreaker(lineList) {
      // 根据传入的线路的id查出对应的breaker 并将第一个breaker的funcType设置为outlet
      const breakerResult = await this.$axios.$post('/models/breaker/lineid/list', lineList);
      if (!breakerResult) {
        this.$message.error('查询breaker失败');
        return;
      }
