...
import * as pathTools from '~/plugins/path';
import * as baseTools from '~/plugins/base';
...
export default {
  ...
  methods: {
    ...
    async getFirstBreaker(busesId) {
      const pathData = new pathTools.路径类();
      const baseData = new baseTools.Base();
      let array = [], deep = 1;
      const links = this.structure.filter(x => baseData(x.from).id === id);
