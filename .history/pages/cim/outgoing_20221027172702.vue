...
export default {
  ...
  methods: {
    ...
    async test() {
      ...
      let [...copy] = structureData;
      for (const structure of copy) {
        console.log(structure.structure.xml);
        // let links = structure.structure.xml.filter(x => new Base(x.from).id === id);
        // console.log(links);
      }


      // function 获取下级设备({ id, array = [], deep = 1 }) {
      //   console.log('2');
      //   if (deep <= 0) {
      //     return array;
      //   }
      //   console.log(lineSvg.structure);
      //   const links = lineSvg.structure.xml.filter(x => new Base(x.from).id === id);
      //   console.log('afterFilter:\n');
      //   console.log(links);
      //   for (const link of links) {
      //     const { id: toId } = new Base(link.to);
      ...
      // let svgData = [];
      // for (const busSvg of busSvgList) {
      //   let filterData = (获取下级设备({ id: busSvg }));
      //   if (filterData.length > 0) {
      //     svgData.push(filterData);
      //   }
      // }
