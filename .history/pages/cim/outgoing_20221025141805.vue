...
export default {
  ...
  methods: {
    ...
    async getFirstBreaker() {
      ...
      //   structureData.push(await (await 路径类.获取路径('PD_10000100_99217')).获取下级设备(busSvg));
      // }
      let deep = 999;
      const structureData = await 路径类.获取路径('PD_10000100_99217');
      const downData = await structureData.获取下级设备('PD_10000100_99217');
      console.log(downData);
      console.log(structureData);
      // for (const lineSvg of lineSvgList) {
      //   for (const busSvg of busSvgList) {
      //     const resultData = await (await 路径类.获取路径(lineSvg)).获取下级设备(busSvg);
      ...
      // const busDevelopMent = structureData.获取下级设备('PD_31100000_64019');
      // console.log(busDevelopMent);
    }
  }
};
</script>
