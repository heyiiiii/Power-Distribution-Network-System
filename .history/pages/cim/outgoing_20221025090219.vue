    ...
    async getFirstBreaker() {
      ...
      class 路径类 {
        ...
        static async 获取路径(svg) {
          ...
          return new 路径类(structureData.result ? structureData.result.xml : []);
        }
        ...
        获取下级设备({ id, array = [], deep = 1 }) {
          ...
          console.log(structureData.result.structure);
          const links = this.xml.filter(x => new Base(x.from).id === id);
      ...
      const structureData = await (await 路径类.获取路径('PD_10000100_41697')).获取下级设备('PD_31100000_64019');
      console.log(structureData);
