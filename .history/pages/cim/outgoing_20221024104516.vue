...
export default {
  ...
  methods: {
    ...
    async getFirstBreaker() {
      // // 拿到母线
      // const { data: busDataList } = await this.$axios.post('/models/allBuses/list');
      // console.log(busDataList);
      // let busSvgArray = [];
      // for (const bus of busDataList.result) {
      //   busSvgArray.push(bus.svg);
      // }
      // console.log('查到母线的所有svg\n');
      // console.log(busSvgArray);
      // 用svg查出breaker
      // const { data: breakerData } = await this.$axios.post('/models/breakerList/svg', busSvgArray);
      // console.log(breakerData.result);
      // 拿到线路的svg
      const { data: lineList } = await this.$axios.post('/models/allLine/list');
      let lineArray = [];
      for (const line of lineList.result) {
        lineArray.push(line.svg);
      }
      console.log(lineArray);
      // 用svg查出breaker
      const { data: lineBreakerData } = await this.$axios.post('/models/breakerList/svg', lineArray);
      console.log(lineBreakerData.result);
    }
  }
};
</script>
