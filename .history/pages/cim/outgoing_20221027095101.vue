    ...
    async getFirstBreaker() {
      ...
      class 路径类 {
        ...
        static async 获取路径(svg) {
          ...
          console.log(structureData.result);
          return new 路径类(structureData.result ? structureData.result : []);
        }
        ...
        获取下级设备({ id, array = [], deep = 1 }) {
          ...
          const links = this.structure.xml.filter(x => new Base(x.from).id === id);
      ...
      let svgData = [];
      for (const lineSvg of lineSvgList) {
        for (const busSvg of busSvgList) {
          let filterData = (await 路径类.获取路径(lineSvg)).获取下级设备({ id: busSvg });
      ...
      for (const key of svgData) {
        filtedBreakerSvg.push(key[0]);
      }
      ...
      // let finalData = [];
      // for (const busSvg of busSvgList) {
      //   let resultData1 = (await 路径类.获取路径('PD_10000100_99217')).获取下级设备({ id: busSvg });
      //   let resultData2 = (await 路径类.获取路径('PD_10000100_1501724403')).获取下级设备({ id: busSvg });
      //   let resultData3 = (await 路径类.获取路径('PD_10000100_41697')).获取下级设备({ id: busSvg });
      //   let resultData4 = (await 路径类.获取路径('PD_10000100_100833')).获取下级设备({ id: busSvg });
      //   finalData.push(resultData1[0]);
      //   finalData.push(resultData2[0]);
      //   finalData.push(resultData3[0]);
      //   finalData.push(resultData4[0]);
      // }
      // console.log(finalData);
      // console.log(lineSvgList);
      // const structureData = await 路径类.获取路径('PD_10000100_99217');
      // const downData = structureData.获取下级设备({ id: 'PD_31100000_64019' });
      // console.log(downData);
      // for (const lineSvg of lineSvgList) {
      //   for (const busSvg of busSvgList) {
      //     const resultData = await (await 路径类.获取路径(lineSvg)).获取下级设备({id:busSvg});
      ...
      //   }
      // }
      // 根据拿到的breaker的svg将breaker的funcType改为outlet。
    },
    async test() {
      // 查出所有线路的svg
      const { data: lineData } = await this.$axios.post('/models/allLine/list');
      ...
      let lineIdList = [];
      for (const line of lineIdList) {
        lineIdList.push(line._id);
      }
      // 根据线路过滤出母线
      const lineBusList = await this.$axios.$post('/models/line/busList', {
        lineSvgList,
        lineIdList
      });
      if (!lineBusList) {
        this.$message.error('根据line的svg查询母线失败');
        return;
      }
      console.log(lineBusList);
    }
  }
};
</script>
