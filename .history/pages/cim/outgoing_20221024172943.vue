...
import line from '~/server/mongo-schema/models/line';
export default {
  ...
  methods: {
    ...
    async getFirstBreaker() {
      ...
      // 查出所有线路的svg
      const { data: lineData } = await this.$axios.post('/models/line/list');
      let lineSvgList = [];
      for (const line of lineData.result) {
        lineSvgList.push(line.svg);
      }
      console.log(lineSvgList);
      const pathData = await new 路径类.获取路径('PD_10000100_1501724403');
