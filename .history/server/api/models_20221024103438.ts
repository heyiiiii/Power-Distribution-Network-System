...
// 根据svg查出breaker
router.post('/breakerList/svg', async (ctx, next) => {
  const svgList: Array<any> = ctx.request.body;
  let svgBreaker = [];
  for (const item of svgList) {
    svgBreaker.push(await Breaker.findOne({
      svg: item,
      areaCode: Number(ctx.headers['area-code'])
    }))
  }
  console.log('根据svg查到的断路器：\n');
  console.log(svgBreaker);
  ctx.body = {
    succ: true,
    result: svgBreaker
  }
