...
router.get('/xml-structures/list', async (ctx, next) => {
  const { data: lineSvgList } = ctx.request.body;
  console.log(lineSvgList);
  let lineStructure = [];
  for (const id in lineSvgList) {
    let structure = await Structure.findOne({ 'metadata.svgId': lineSvgList[id].svg }).lean();
