...
router.post('/getBreakerList/svgid', async (ctx, next) => {
  const svgIdList = ctx.request.body;
  let breakerData = [];
  for (const svgid of svgIdList) {
    // breakerData.push(await Breaker.findOne({ 'svg': svgid }).lean());
    // breakerData.push(
    //   await Breaker.findOne({
    //     svg: svgid,
    //     areaCode: Number(ctx.headers['area-code'])
    //   }))
    breakerData.push(await Breaker.findOne().where('svg').in(svgid).exec())
  }
  ctx.body = {
    succ: true,
    result: breakerData
