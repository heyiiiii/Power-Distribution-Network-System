...
// 根据bus的id查询对应的breaker,并将第一个开关类型设置为出线类型
router.post('/breaker/busidList', async (ctx) => {
  const busIdList = ctx.request.body;
  let breakerList = [];
  for (const busId of busIdList) {
    breakerList.push(await Breaker.where('line').in(busId).exec());
    breakerList[0].funcType = 'outlet'
  }
  ctx.body = {
    succ: true,
    result: breakerList
