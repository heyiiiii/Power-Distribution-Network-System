...
router.post('/updateBreaker/svgList', async (ctx, next) => {
  const breakerSvgs = ctx.request.body.afterFilterSvg
  console.log(breakerSvgs);
  ctx.body = {
    succ: true,
...
//  根据多条svg进行线路结构的查询 --Jyz
router.post('/xml-structures/list', async (ctx, next) => {
  const svgList = ctx.request.body.lineSvgList;
  console.log(svgList);
  let lineStructure = [];
  for (const id of svgList) {
    let structure = await Structure.findOne({ 'metadata.svgId': id }).lean();
    let stream = getStructureBucket().openDownloadStream(structure!._id);
    let data = await readFile(stream);
    let result = JSON.parse(data.toString());
    lineStructure.push(result);
  }
  if (lineStructure === null) {
    ctx.body = { succ: false, result: '查询失败' };
    return next();
  }
  ctx.body = { succ: true, result: lineStructure };
});
